...
public class PermissionsEx extends JavaPlugin implements NativeInterface {
	...
	public void onEnable() {
		...
		try {
			...
			if (getConfiguration().useNetEvents()) {
				Plugin SQNetEvents = getServer().getPluginManager().getPlugin("NetEvents");
				if (SQNetEvents != null && SQNetEvents.isEnabled()) {
					SQNetEvents netEvents = (SQNetEvents) SQNetEvents;
	...
	public UUID getServerUUID() {
		return netEvents == null ? null : netEvents.getUUID();
	}

	@Override
	public void callEvent(PermissionEvent event) {
		if (netEvents != null) {
			netEvents.getServer().getPluginManager().callEvent(event);
		} else {
