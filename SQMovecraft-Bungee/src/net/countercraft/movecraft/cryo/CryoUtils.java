...
public class CryoUtils {
	...
	/*boolean isCryoTybe(Sign s){
		if(s.getLine(0).equals(KEY_LINE)){
			Block b = s.getBlock();
			if(test(b, BlockFace.UP)&& test(b, BlockFace.EAST) && test(b, BlockFace.WEST) && test(b, BlockFace.SOUTH) && test(b, BlockFace.NORTH)){
				Block b2 = s.getBlock().getRelative(BlockFace.DOWN);
				if(test(b2, BlockFace.DOWN)&& test(b2, BlockFace.EAST) && test(b2, BlockFace.WEST) && test(b2, BlockFace.SOUTH) && test(b2, BlockFace.NORTH)){
					return true;
				}
			}
		}
	...
	/*private boolean test(Block b, BlockFace relative){
