...
public class TeleportTask {
	public static boolean worldJump(Player pilot, Craft c, Location locto){
		...
		//calculate the difference in x and difference in y of the current location to the target location
		...
		//	statText.mkdirs();
		//}
		//String filePrefix = pilot.getName() + "@" + System.currentTimeMillis();
		...
		//w.newLine();
		//w.write("Target Coords: " + locto.getX() + "," + locto.getY() + "," + locto.getZ());
		//w.newLine();
		...
		//.write(wrld.getBlockTypeIdAt(l.getX(), l.getY(), l.getZ()) + "@" + l.getX()+ "," + l.getY() + "," + l.getZ());
		//.newLine();
		//}
	...
	private static void processSingleBlock(MovecraftLocation oldLoc, Location newLoc, World w, int oldID, byte oldData){
		...
		//.write("Id updated to " + oldID + " and data updated to " + oldData + " at " + newLoc.getX() + "," + newLoc.getY() + "," + newLoc.getZ());
		//wrt.newLine();
		...
		/*if (oldID == 121){
			//wrt.write("Found to be a storage crate.");
			//wrt.newLine();
			Inventory inventory = StorageChestItem.getInventoryOfCrateAtLocation(oldLoc, w);
			StorageChestItem.removeInventoryAtLocation(w, oldLoc);
			StorageChestItem.setInventoryOfCrateAtLocation(inventory, new MovecraftLocation(newLoc.getBlockX(), newLoc.getBlockY(), newLoc.getBlockZ()), newLoc.getWorld());
		}*/
