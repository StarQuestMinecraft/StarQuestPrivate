		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				if (Arrays.binarySearch(inventoryIDs, id) >= 0) {
					Inventory contents;
					/*
					 * boolean continuing = true; while (continuing){ String
					 * continuingstring = msgin.readUTF(); if
					 * (continuingstring.equals("continue")){
					 * System.out.println("continuing."); int itemid =
					 * msgin.readInt(); int itemdata = msgin.readInt(); int
					 * amount = msgin.readInt(); int slot = msgin.readInt();
					 * System.out.println("ItemStack: " + itemid + "," +
					 * itemdata + "," + amount + "," + slot); ItemStack stack =
					 * new ItemStack(itemid, amount, (short) itemdata);
					 * contents.add(new ItemStackHolder(stack,slot)); } else if
					 * (continuingstring.equals("stop")){
					 * System.out.println("stopping."); continuing = false; } }
					 */
			...
			BungeeCraftConstructor.calculateLocationAndBuild(targetworld, Xcoord, Ycoord, Zcoord, oldX, oldY, oldZ, cName, pName, blockloclist, bedspawnNames,
					playersOnShip);
		} catch (IOException e) {
			e.printStackTrace();
		}
